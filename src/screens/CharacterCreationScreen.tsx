...
import { CLASSES_LIST, RACES_LIST, SKILLS_LIST, WEAPON_TEMPLATES, ARMOR_TEMPLATES, getSpellSlotsForClass, BACKGROUNDS_LIST, getHitDieType, getSpellLimit, getArmorCategory, isProficientInItem, getRaceStatBonuses } from '../utils/dndRules';
...
export const CharacterCreationScreen: React.FC<CharacterCreationScreenProps> = ({
  ...
  const getDefaultSkillsFor = (raceName: string, bgName: string) => {
    ...
    if (rLower.includes('tabaxi')) {
      fixedRacialSkills.push('Furtividade');
      fixedRacialSkills.push('Percepção');
    }
  ...
  const handleAssignStatValue = (targetStat: keyof BaseStats, value: number) => {
    const duplicateStat = (Object.keys(stats) as Array<keyof BaseStats>).find(s => s !== targetStat && stats[s] === value);
    setStats(prev => {
      const updated = {
        ...prev
      };
    ...
    setSelectedSkills(prev => {
      ...
      if (prev.includes(skill)) {
        ...
        if (currentManualSelected.length >= totalManualAllowed) {
          Alert.alert('Limite de Perícias', `Você já selecionou o máximo de ${totalManualAllowed} perícias permitidas.`);
          return prev;
        }
        if (!isPhase1Complete) {
          // Phase 1: Only class skills are allowed
          if (!isClassSkill) {
            Alert.alert('Fase de Classe', `Por favor, primeiro selecione suas ${classRules.limit} perícias da lista da sua classe.`);
            return prev;
          }
        } else {
  ...
  return <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        {/* STEP 1: Basic Info */}
        {step === 1 && <View style={styles.stepCard}>
            <Text style={styles.stepTitle}>Informações Básicas</Text>
            
            <Text style={styles.label}>Nome do Herói</Text>
            <TextInput style={styles.input} placeholder="ex: Lancelot, Aragorn" placeholderTextColor={colors.borderHighlight} value={name} onChangeText={setName} />

            <Text style={styles.label}>Imagem do Personagem</Text>
            <TouchableOpacity style={styles.imagePickerBtn} onPress={pickImage}>
              {imageUrl ? <Image source={{
            uri: imageUrl
          }} style={styles.characterImagePreview} /> : <View style={styles.imagePlaceholder}>
                  <Ionicons name="camera-outline" size={32} color={colors.borderHighlight} />
            ...
            <View style={styles.pickerRowWrap}>
              {CLASSES_LIST.map(c => <TouchableOpacity key={c.name} style={[styles.pickerBtnWrap, selectedClass === c.name && styles.pickerBtnActive]} onPress={() => {
          ...
          return <View style={{
            ...
            {selectedClassObj?.subclasses && selectedClassObj.subclasses.length > 0 && <>
                ...
                <View style={styles.pickerRowWrap}>
                  {selectedClassObj.subclasses.map(sub => <TouchableOpacity key={sub} style={[styles.pickerBtnWrap, {
              width: '48%'
            }, selectedSubclass === sub && styles.pickerBtnActive]} onPress={() => setSelectedSubclass(sub)}>
            ...
            return <View style={{
              ...
              marginBottom: 12
                ...
                fontSize: 11,
                fontWeight: '700'
              }}>Benefício do Arquétipo:</Text>
                      <Text style={{
                color: colors.textSecondary,
                fontSize: 10,
                marginTop: 2,
                lineHeight: 14
              }}>{subclassBenefit}</Text>
                    </View>;
          })()}
              </>}
          ...
          const raceGroups: Record<string, string[]> = {
            ...
            'Orc': ['Orc', 'Meio-Orc'],
            'Draconato': ['Draconato Vermelho (Fogo)', 'Draconato Latão (Fogo)', 'Draconato Ouro (Fogo)', 'Draconato Azul (Eletricidade)', 'Draconato Bronze (Eletricidade)', 'Draconato Cobre (Ácido)', 'Draconato Preto (Ácido)', 'Draconato Verde (Veneno)', 'Draconato Branco (Frio)', 'Draconato Prata (Frio)'],
          ...
          const visibleGroups = isExpanded ? [...coreGroupNames, ...extraGroupNames] : coreGroupNames;
          return <View style={{
            marginBottom: 10
          }}>
                  {/* Unified Race Grid */}
                ...
                return <TouchableOpacity key={group} style={[styles.pickerBtnWrap, {
                  width: '31%',
                  marginBottom: 6
                }, isActive ? styles.pickerBtnActive : null]} onPress={() => {
                  ...
                  setSelectedSkills(getDefaultSkillsFor(firstInGroup, selectedBackground));
                    ...
                    <TouchableOpacity style={[styles.pickerBtnWrap, {
                width: '31%',
                marginBottom: 6,
                borderColor: colors.accentAmber,
                backgroundColor: 'rgba(245, 158, 11, 0.03)'
                  ...
                  {currentGroup && raceGroups[currentGroup].length > 1 && <View style={{
              ...
              }}>Sub-raça / Origem</Text>
                      <View style={styles.pickerRowWrap}>
                        {raceGroups[currentGroup].map(sub => {
                  const isSel = selectedRace === sub;
                  return <TouchableOpacity key={sub} style={[styles.pickerBtnWrap, {
                    width: '48%',
                    paddingVertical: 6,
                    marginBottom: 4
                  }, isSel && styles.pickerBtnActive]} onPress={() => {
                    setSelectedRace(sub);
              ...
              const bonusText = Object.entries(bonuses).map(([stat, val]) => `${stat.toUpperCase()} +${val}`).join(', ');
              let traitText = '';
              const rLower = selectedRace.toLowerCase();
              ...
              return <View style={{
                ...
                marginTop: 6
                  ...
                  fontSize: 11,
                  fontWeight: '700'
                }}>Benefícios de {selectedRace}:</Text>
                        <Text style={{
                  color: colors.textMuted,
                  fontSize: 9,
                  fontWeight: '600',
                  marginTop: 1
                }}>Bônus: {bonusText || 'Nenhum'}</Text>
                        {traitText ? <Text style={{
                  color: colors.textSecondary,
                  fontSize: 10,
                  marginTop: 3,
                  lineHeight: 14
            ...
            <View style={styles.pickerRowWrap}>
              {BACKGROUNDS_LIST.map(bg => <TouchableOpacity key={bg.name} style={[styles.pickerBtnWrap, {
            width: '48%'
          }, selectedBackground === bg.name && styles.pickerBtnActive]} onPress={() => {
          ...
          return <View style={{
            ...
            marginTop: 6
              ...
              fontSize: 11,
              fontWeight: '700'
            }}>Habilidade de {bg.name.split(' (')[0]}:</Text>
                  <Text style={{
              color: colors.accentAmber,
              fontSize: 9,
              fontWeight: '600',
              marginTop: 1
            }}>Perícias Iniciais: {bg.skills.join(', ')}</Text>
                  <Text style={{
              color: colors.textMuted,
              fontSize: 9,
              fontWeight: '600',
              marginTop: 1
            }}>Recurso: {bg.featureName}</Text>
                  <Text style={{
              color: colors.textSecondary,
              fontSize: 10,
              marginTop: 3,
            ...
            <View style={styles.levelRow}>
              {[1, 2, 3, 4, 5, 6].map(lvl => <TouchableOpacity key={lvl} style={[styles.lvlBtn, level === lvl && styles.lvlBtnActive]} onPress={() => {
            setLevel(lvl);
        ...
        return <View style={styles.stepCard}>
              ...
              <View style={styles.methodSelectorRow}>
                <TouchableOpacity style={[styles.methodBtn, statMethod === 'standard' && styles.methodBtnActive]} onPress={() => handleSelectStatMethod('standard')}>
                  <Text style={[styles.methodBtnLabel, statMethod === 'standard' && styles.methodBtnLabelActive]}>Padrão</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.methodBtn, statMethod === 'pointbuy' && styles.methodBtnActive]} onPress={() => handleSelectStatMethod('pointbuy')}>
              ...
              {statMethod === 'pointbuy' && <View style={{
            flexDirection: 'row',
            ...
            marginBottom: 12
              ...
              borderColor: colors.accentEmerald
                ...
                fontWeight: '800',
                fontSize: 12
              }}>{27 - pointsSpent} / 27 pts</Text>
                  </View>
                </View>}
              {statMethod === 'roll' && <View style={{
            flexDirection: 'row',
            justifyContent: 'space-between',
            alignItems: 'center',
            marginBottom: 12
                  ...
                  <TouchableOpacity style={styles.rollAllBtn} onPress={rollAllDice}>
                    <Ionicons name="dice" size={16} color={colors.background} style={{
                marginRight: 4
              }} />
              ...
              {(Object.keys(stats) as Array<keyof BaseStats>).map(stat => <View key={stat} style={styles.statAssignRow}>
                ...
                return <TouchableOpacity key={val} style={[styles.arrayValBtn, isSelected && styles.arrayValBtnActive]} onPress={() => handleAssignStatValue(stat, val)}>
                  ...
                  {statMethod === 'pointbuy' && <View style={styles.pointBuySelector}>
                  ...
                  {statMethod === 'roll' && <View style={styles.pointBuySelector}>
                      <View style={[styles.pbValueWrap, {
                width: 40
              }]}>
                        <TextInput style={[styles.pbValue, {
                  textAlign: 'center',
                  padding: 0
                }]} keyboardType="numeric" value={String(stats[stat])} onChangeText={val => {
                  ...
                  setStats(prev => ({
                    ...
                    [stat]: num
                      ...
                      <TouchableOpacity style={styles.rollBtn} onPress={() => rollDiceForStat(stat)}>
                        <Ionicons name="dice" size={20} color={colors.accentAmber} />
                      </TouchableOpacity>
                    </View>}
                </View>)}
            </View>;
        ...
        if (rLower.includes('goliath')) fixedRacialSkills.push('Atletismo');
        const currentManualSelected = selectedSkills.filter(s => !bgSkills.includes(s) && !fixedRacialSkills.includes(s));
        let classUsed = 0;
        let raceUsed = 0;
        for (const s of currentManualSelected) {
          if (classRules.list.includes(s) && classUsed < classRules.limit) {
        ...
        return <View style={styles.stepCard}>
            ...
            paddingBottom: 8
              ...
              flex: 1,
              paddingBottom: 6,
              borderBottomWidth: !isPhase1Complete ? 2 : 0,
              borderBottomColor: colors.accentAmber
            }}>
                  <Text style={{
                color: !isPhase1Complete ? '#FBBF24' : colors.textMuted,
                fontSize: 11,
                fontWeight: '700'
              }}>
              ...
              <Text style={styles.stepDesc}>
                {!isPhase1Complete ? `Selecione as perícias de treino da sua classe (${selectedClass}). Escolha mais ${classRules.limit - classUsed}.` : hasRacialPhase && raceUsed < allowedRacialSlots ? `Fase da Classe concluída! Agora selecione ${allowedRacialSlots - raceUsed} perícias adicionais concedidas por sua raça (${selectedRace}).` : "Todas as proficiências de classe e raça foram selecionadas com sucesso!"}
              </Text>
                ...
                toggleSkill(skill);
                  ...
                  flex: 1
                    ...
                    marginTop: 1
                        ...
                        {isRacialFixed && <Text style={{
                    color: colors.accentAmber,
                    fontSize: 7,
                    fontWeight: '700',
                    marginTop: 1
                  }}>RAÇA ({selectedRace.split(' ')[0]})</Text>}
                        {!isFromBg && !isRacialFixed && isClassSkill && <Text style={{
                    color: colors.accentEmerald,
                    fontSize: 7,
                    fontWeight: '700',
                    marginTop: 1
        ...
        {step === 4 && <View style={styles.stepCard}>
          ...
          return <TouchableOpacity key={w.name} style={[styles.equipOption, selectedWeaponIdx === idx && styles.equipOptionActive]} onPress={() => {
            setSelectedWeaponIdx(idx);
            if (w.handedness === '2 Mãos') {
              setHasShield(false);
            }
          }} activeOpacity={0.8}>
                  <Ionicons name={"sword" as any} size={16} color={selectedWeaponIdx === idx ? colors.accentAmber : colors.borderHighlight} style={{
              marginRight: 10
            }} />
                  <View style={{
              flex: 1
                    ...
                    <View style={styles.creationEquipHeaderRow}>
                      ...
                      <View style={[styles.profSmallBadge, isProf ? styles.profSmallBadgeActive : styles.profSmallBadgeInactive]}>
                        <Text style={[styles.profSmallBadgeText, isProf ? styles.profSmallBadgeTextActive : styles.profSmallBadgeTextInactive]}>
                    ...
                    {w.properties.length > 0 && <Text style={styles.equipProperties}>
          ...
          return <TouchableOpacity key={a.name} style={[styles.equipOption, selectedArmorIdx === idx && styles.equipOptionActive, !meetsStrength && styles.equipOptionDisabled]} onPress={() => {
            if (!meetsStrength) {
              Alert.alert('Requisito de Força', `Esta armadura requer Força ${a.strengthReq}. Sua Força é ${currentStrength}. Você terá desvantagens se equipá-la.`);
            }
            setSelectedArmorIdx(idx);
          }} activeOpacity={0.8}>
                  <Ionicons name={"shirt" as any} size={16} color={selectedArmorIdx === idx ? colors.accentAmber : colors.borderHighlight} style={{
              marginRight: 10
            }} />
                  <View style={{
              flex: 1
          ...
          return <View style={[styles.shieldToggleRow, isWeapon2H && styles.shieldToggleRowDisabled]}>
                  <View style={{
              flex: 1
            }}>
                    <View style={{
                ...
                alignItems: 'center'
                      ...
                      <View style={[styles.profSmallBadge, isShieldProf ? styles.profSmallBadgeActive : styles.profSmallBadgeInactive, {
                  marginLeft: 6
                }]}>
                        <Text style={[styles.profSmallBadgeText, isShieldProf ? styles.profSmallBadgeTextActive : styles.profSmallBadgeTextInactive]}>
                          {isShieldProf ? 'Treinado' : 'Sem Treino'}
                        </Text>
                      </View>
                    </View>
                    <Text style={styles.shieldSub}>
                      {isWeapon2H ? 'Desabilitado: Usando arma de duas mãos.' : 'Disponível apenas para armas de 1 mão ou versáteis.'}
                    </Text>
                  </View>
        ...
        {step === 5 && <View style={styles.stepCard}>
            ...
            {isSpellcaster ? <View>
                {maxSpellLevel > 0 ? <View style={styles.spellLimitBanner}>
                  ...
                  </View> : <View style={styles.spellLimitBanner}>
                    <Text style={styles.spellLimitText}>
                      Neste nível você só pode conjurar truques (Nível 0).
                    </Text>
                  </View>}

                {availableSpells.length === 0 ? <Text style={styles.emptyText}>Nenhuma magia disponível para sua classe neste nível.</Text> : availableSpells.map(spell => {
            const isSelected = selectedSpells.includes(spell.name);
            const isCantrip = spell.level === 0;
            return <TouchableOpacity key={spell.name} style={[styles.spellCard, isSelected && styles.spellCardActive]} onPress={() => toggleSpellSelection(spell.name)} activeOpacity={0.8}>
                ...
                <Ionicons name="flash-off" size={44} color={colors.borderHighlight} style={{
            marginBottom: 12
          }} />
      ...
      <View style={styles.footer}>
        {step > 1 ? <TouchableOpacity style={styles.footerBackBtn} onPress={() => setStep(step - 1 as StepType)}>
            <Text style={styles.footerBackBtnText}>Voltar</Text>
          </TouchableOpacity> : <View style={{
        flex: 0.8
      }} />}

        {step < 5 ? <TouchableOpacity style={styles.footerNextBtn} onPress={() => setStep(step + 1 as StepType)}>
            <Text style={styles.footerNextBtnText}>Avançar</Text>
            <Ionicons name="arrow-forward" size={16} color={colors.background} style={{
          marginLeft: 6
        }} />
          </TouchableOpacity> : <TouchableOpacity style={[styles.footerNextBtn, styles.saveBtn]} onPress={handleSaveCharacter}>
            <Ionicons name="checkmark-circle" size={16} color={colors.background} style={{
          marginRight: 6
        }} />
